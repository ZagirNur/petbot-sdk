...
public class ReplyBuilderByChatId {
    ...
    /**
     * Отправляет документ в чат. Используется для выгрузок (CSV и т.п.).
     */
    public void sendDocument(byte[] content, String fileName) {
        ...
        try {
            long sentMessageId = bot.execute(sendDocument).getMessageId().longValue();
            doWithSentMessageId.forEach(consumer -> consumer.accept(sentMessageId));
        } catch (TelegramApiException e) {
            throw new RuntimeException(e);
        }
    }
     ...
     */
    public void deleteKeyboard(Long messageIdToEdit) {
        EditMessageReplyMarkup method = EditMessageReplyMarkup.builder()
                .chatId(chatId)
                .messageId(messageIdToEdit.intValue())
                .replyMarkup(null)
                .build();
